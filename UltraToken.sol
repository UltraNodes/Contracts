...
contract UltraToken is ERC20Permit, Ownable {
    using SafeMath for uint256;
    ...
    ERC20("UltraFinance", "ULTRA", 9) 
    ERC20Permit("ULTRA") 
    {

        IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(0x60aE616a2155Ee3d9A68541Ba4544862310933d4);
        uniswapV2Router = _uniswapV2Router;
        uniswapV2Pair = IUniswapV2Factory(_uniswapV2Router.factory()).createPair(address(this), uniswapV2Router.WAVAX());
        _setAutomatedMarketMakerPair(address(uniswapV2Pair), true);

        initialSupply = 1000000000*1e9;
        maxTransactionAmount = initialSupply * 1 / 1000; // 0.1% maxTransactionAmountTxn
    ...
    function applyLimits() external onlyOwner returns (bool){
        limitsInEffect = true;
        return true;
    }
            ...
            if (
                ...
                if (transferDelayEnabled){
                    if (from != owner() && automatedMarketMakerPairs[from]){
                        require(_holderLastTransferTimestamp[tx.origin] < block.number, "_transfer:: Transfer Delay enabled.  Only one tx per block allowed.");
    ...
    function swapBack() private {
        ...
        if(liquidityTokens > 0 && ethForLiquidity > 0){
            ...
            emit SwapAndLiquify(amountToSwapForAVAX, ethForLiquidity, tokensForLiquidity);
        }
    ...
    function withdrawAvaxPool() external onlyOwner() {
        bool success;
    ...
    function transferToTreasury(address _tokenAddr, uint _amount) public onlyOwner {
        IERC20(_tokenAddr).transfer(treasuryWallet, _amount);
    }
